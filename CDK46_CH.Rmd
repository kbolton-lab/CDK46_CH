...
#         collection_year = format(SampleCollectionDate, "%Y"),
#         geo_loc_name = "United States of America",
#         sex = Sex,
#         tissue = "Blood"
#     ) %>%
#     dplyr::select(
#         sample_name, sample_title, organism, isolate, age, biomaterial_provider, collection_date, collection_year, geo_loc_name, sex, tissue
#     ) %>%
...
data_to_plot %>%
    ungroup() %>%
    group_by(Cohort, Trilaciclib) %>%
    summarise(
        mean_change_in_VAF = mean(change_in_VAF),
        mean_change_in_VAF_per_month = mean(change_in_VAF_per_month),
        change_in_days = mean(change_in_days),
    )
