...
# Removing Samples with Low Sequencing Depths (May give inaccurate results)
low_depth_deid <- c("1163006", "1148005", "1114002", "1131013", "1117006", "2022-3022", "2028-3011", "2009-3003", "2810-3003", "2405-3002", "2703-3002")
g1_df <- g1_df %>%
    filter(!(DeID %in% low_depth_deid))
g1_tp <- g1_tp %>%
    filter(!(DeID %in% low_depth_deid))
...
model_change_in_VAF_per_month <- rbind(
    glm(
        formula = change_in_VAF_per_month ~ Trilaciclib + Age + starting_clone_vaf,
        data = data_to_plot %>% filter(Gene_Class == "DDR")
    ) %>% sjPlot::get_model_data(type = "est"),
    glm(
        formula = change_in_VAF_per_month ~ Trilaciclib + Age + starting_clone_vaf,
        data = data_to_plot %>% filter(Gene_Class == "DDR") %>% filter(Cohort != "Untreated")
    ) %>% sjPlot::get_model_data(type = "est")
) %>% filter(grepl("Trilaciclib", term))

model_change_in_VAF <- rbind(
    glm(
        formula = change_in_VAF ~ Trilaciclib + Age + starting_clone_vaf,
        data = data_to_plot %>% filter(Gene_Class == "DDR")
    ) %>% sjPlot::get_model_data(type = "est"),
    glm(
        formula = change_in_VAF ~ Trilaciclib + Age + starting_clone_vaf,
...
supp_fig1 <- data_to_plot %>%
    ...
    do.call(geom_signif, default_sig_data(data_to_plot, 5.5, 1, 2, model_change_in_VAF_per_month$p.value[1], model_change_in_VAF_per_month$p.stars[1], show_values = FALSE)) +
    do.call(geom_signif, default_sig_data(data_to_plot, 6.5, 1, 3, model_change_in_VAF_per_month$p.value[2], model_change_in_VAF_per_month$p.stars[2], show_values = FALSE)) +
    do.call(geom_signif, default_sig_data(data_to_plot, 7.5, 2, 3, model_change_in_VAF_per_month$p.value[3], model_change_in_VAF_per_month$p.stars[3], show_values = FALSE))
    # do.call(geom_signif, default_sig_data(data_to_plot, 6, 1, 2, model_change_in_VAF$p.value[1], model_change_in_VAF$p.stars[1], show_values = TRUE)) +
    # do.call(geom_signif, default_sig_data(data_to_plot, 7, 1, 3, model_change_in_VAF$p.value[2], model_change_in_VAF$p.stars[2], show_values = TRUE)) +
    # do.call(geom_signif, default_sig_data(data_to_plot, 8, 2, 3, model_change_in_VAF$p.value[3], model_change_in_VAF$p.stars[3], show_values = TRUE))
supp_fig1
