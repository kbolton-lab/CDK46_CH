...
bio_fig4_unfiltered_data <- fread(paste0(data_file_path, "cdk46_ch_pilot_harvard.csv"))
```
...
g1_tp <- g1_tp %>%
    left_join(
        g1_df %>%
            dplyr::select(
                key, Gene, VariantClass, gene_aachange, oncoKB, oncoKB_reviewed,
...
g1_tp$Trilaciclib <- factor(g1_tp$Trilaciclib, levels = c("Untreated", "Placebo", "Trilaciclib"))
...
g1_tp$Cohort <- factor(g1_tp$Cohort, levels = c("Untreated", "SCLC", "CRC", "TNBC"))
...
# Spider Plots
```{r spider_plots, include=TRUE, message = FALSE, warning = FALSE}
avg_change <- g1_tp %>% group_by(Gene_Class, Cohort) %>% summarise(n = mean(change_in_VAF))

g1_tp %>%
    filter(Gene_Class == "DTA" | Gene_Class == "DDR") %>%
    filter(Cohort == "Untreated") %>%
    filter(change_in_days <= 1000) %>%
    mutate(
    ...
    facet_wrap(~Gene) + 
    #facet_wrap(Gene~Cohort) +#, ncol = 3, nrow = 2) +
    theme(
...
g1_mut_table %>%
    filter(whichdraw == "PreTx") %>%
    mutate(CH_Binary = ifelse(rowSums(across(where(is.integer)), na.rm = TRUE) > 0, 1, 0)) %>%
...
pdf(paste0(figures_save_path, "Figure1B.pdf"), width = 14, height = 8)
fig1b
dev.off()
```
...
pdf(paste0(figures_save_path, "Figure1C.pdf"), width = 8, height = 5)
draw(
    p,
...
fig1d <- df_to_plot %>%
    ...
    scale_fill_nejm()
pdf(paste0(figures_save_path, "Figure1D.pdf"), width = 8, height = 10)
fig1d
dev.off()
```
...
model_treatment_binary_gene <- map_dfr(c(gene_list), function(gene){
    glm(
        formula = as.formula(paste(gene, "~ Treatment + Age")),
        family = binomial(link = "logit"),
        data = g1_mut_table %>%
            filter(whichdraw == "PostTx") %>%
            mutate(Treatment = ifelse(Trilaciclib == "Untreated", "Healthy Control", "Post-Treatment"))
    ) %>%
    sjPlot::get_model_data(type = "est") %>%
    mutate(Gene = gene)
})
model_treatment_binary_gene <- model_treatment_binary_gene %>% filter(term != "Age")
...
g1_mut_table <- g1_mut_table %>%
    mutate(
        ...
        Treatment = ifelse(Trilaciclib == "Untreated", "Healthy Control", "Post-Treatment"),
        Treatment = factor(Treatment, levels = c("Healthy Control", "Post-Treatment"))
    )
...
fig1e <- g1_mut_table %>%
    ...
    #scale_fill_nejm() +
    scale_fill_manual(values = c("Healthy Control" = "#BC3C29FF", "Post-Treatment" = "#0072B2")) +
    scale_color_nejm()
    do.call(geom_signif, default_sig(0.32, 3.75, 4.25, 0, "***", tip_length = 0.05, size = 1.5, show_values = FALSE, hide_ns = FALSE)) +
    do.call(geom_signif, default_sig(0.37, 4.75, 5.25, 0, "***", tip_length = 0.05, size = 1.5, show_values = FALSE, hide_ns = FALSE)) +
    do.call(geom_signif, default_sig(0.20, 5.75, 6.25, 0, "*", tip_length = 0.05, size = 1.5, show_values = FALSE, hide_ns = FALSE))
fig1e
#pdf(paste0(figures_save_path, "Figure1C.pdf"), width = 8, height = 5)
pdf(paste0(figures_save_path, "Figure1E.pdf"), width = 10, height = 6)
fig1e
dev.off()
```
...
model_treatment_gene_class <- map_dfr(c("DTA_Mutations", "DDR_Mutations"), function(gene_class){
    rbind(
        ...
        sjPlot::get_model_data(type = "est") %>%
        mutate(GeneClass = ifelse(gene_class == "DTA_Mutations", "DTA", "DDR"))
    )
})
...
fig1e_cont <- g1_mut_table %>%
    ...
    mutate(
        ...
        #Gene = ifelse(Gene == "DTA_Mutations", "**DTA**: *DNMT3A, TET2, ASXL1*", "**DDR**: *TP53, PPM1D, CHEK2*"),
        #Gene = factor(Gene, levels = c("**DTA**: *DNMT3A, TET2, ASXL1*", "**DDR**: *TP53, PPM1D, CHEK2*"))
    ) %>%
    ggplot(
        aes(
            x = Gene,
    ...
    do.call(geom_signif, default_sig(0.65, 0.75, 1.25, model_treatment_gene_class$p.value[1], model_treatment_gene_class$p.stars[1], tip_length = 0.1, size = 0.5, show_values = FALSE, hide_ns = FALSE)) +
    do.call(geom_signif, default_sig(0.65, 1.75, 2.25, round(model_treatment_gene_class$p.value[2], 2), model_treatment_gene_class$p.stars[2], tip_length = 0.1, size = 0.5, show_values = TRUE, hide_ns = FALSE))
pdf(paste0(figures_save_path, "Figure1E_Continued.pdf"), width = 10, height = 6)
fig1e_cont
dev.off()
```
...
fig1f <- df_to_plot %>%
    ...
    geom_text(data = counts, aes(label = n, y = 0.06), size = 10, position = position_dodge(width = 0.8)) +
    labs(
        y = "Growth Rate", 
        x = '', 
        title = "",
    ...
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot, 5, 0.75, 1, model_growth_rate_class$p.value[1], model_growth_rate_class$p.stars[1], show_values = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 0.75, 1.25, model_growth_rate_class$p.value[2], model_growth_rate_class$p.stars[2], show_values = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 1, 1.25, model_growth_rate_class$p.value[5], model_growth_rate_class$p.stars[5], show_values = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 5, 1.75, 2, model_growth_rate_class$p.value[3], model_growth_rate_class$p.stars[3], show_values = TRUE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 0.5))
pdf(paste0(figures_save_path, "Figure1F.pdf"), width = 14, height = 14)
fig1f
dev.off()
```
...
fig2c <- df_to_plot %>%
    ...
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 0.5))
pdf(paste0(figures_save_path, "Figure2C.pdf"), width = 10, height = 8)
fig2c
dev.off()
```
...
fig2d <- df_to_plot %>%
    ...
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 0.5))
pdf(paste0(figures_save_path, "Figure2D.pdf"), width = 10, height = 8)
fig2d
dev.off()
```
...
model_growth_rate_gene <- purrr::cross_df(list(
    ...
    #gene = c("TP53", "PPM1D", "CHEK2", "DNMT3A", "TET2", "ASXL1"),
    comparison = c("all", "no_untreated")
)) %>%
...
df_to_plot <- df_to_plot %>%
    mutate(
        ...
        Gene = factor(Gene, levels = c("*TP53*", "*PPM1D*", "*CHEK2*", "*DNMT3A*", "*TET2*", "*ASXL1*"))
    )
...
fig2e <- df_to_plot %>%
    ...
    geom_boxplot(outlier.shape = NA) +
    geom_point(position = position_jitterdodge(jitter.width=0.2), alpha = 0.3) +
    geom_text(data = counts_genes, aes(label = n, y = 0.06), size = 10, position = position_dodge(width = 0.8)) +
    labs(
        y = "Growth Rate", 
        x = '', 
        title = "",
    ...
    panel_theme_basic +
    theme(
        axis.text.x = element_markdown(size = 32),
    ...
    scale_y_continuous(trans = scales::pseudo_log_trans(0.001), breaks = c(-0.05, -0.01, -0.005, 0, 0.005, 0.01, 0.05), limit = c(NA, 1.5)) +
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 5, 0.75, 1, model_growth_rate_gene$p.value[1], model_growth_rate_gene$p.stars[1], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 6, 0.75, 1.25, model_growth_rate_gene$p.value[2], model_growth_rate_gene$p.stars[2], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 7, 1, 1.25, model_growth_rate_gene$p.value[7], model_growth_rate_gene$p.stars[7], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 5, 1.75, 2, model_growth_rate_gene$p.value[3], model_growth_rate_gene$p.stars[3], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) + 
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 6, 1.75, 2.25, model_growth_rate_gene$p.value[4], model_growth_rate_gene$p.stars[4], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 7.1, 2, 2.25, model_growth_rate_gene$p.value[8], model_growth_rate_gene$p.stars[8], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 5, 2.75, 3, model_growth_rate_gene$p.value[5], model_growth_rate_gene$p.stars[5], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 6, 2.75, 3.25, model_growth_rate_gene$p.value[6], model_growth_rate_gene$p.stars[6], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 7.1, 3, 3.25, model_growth_rate_gene$p.value[9], model_growth_rate_gene$p.stars[9], show_values = TRUE, hide_ns = FALSE, textsize = 10, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*DNMT3A*"), 6, 4, 4.25, "1.528048e-02", "*", show_values = FALSE, hide_ns = FALSE, textsize = 6))
pdf(paste0(figures_save_path, "Figure2E.pdf"), width = 16, height = 12)
fig2e
dev.off()
...
ext_fig_tp53 <- df_to_plot %>%
    ...
    scale_y_continuous(trans = scales::pseudo_log_trans(0.001), breaks = c(-0.05, -0.01, -0.005, 0, 0.005, 0.01, 0.05), limit = c(NA, 1.5)) +
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot, 5, 0.75, 1, model_growth_rate_variant_class$p.value[1], model_growth_rate_variant_class$p.stars[1], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 0.75, 1.25, model_growth_rate_variant_class$p.value[2], model_growth_rate_variant_class$p.stars[2], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 1, 1.25, model_growth_rate_variant_class$p.value[5], model_growth_rate_variant_class$p.stars[5], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 5, 1.75, 2, model_growth_rate_variant_class$p.value[3], model_growth_rate_variant_class$p.stars[3], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 1.75, 2.25, model_growth_rate_variant_class$p.value[4], model_growth_rate_variant_class$p.stars[4], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 2, 2.25, model_growth_rate_variant_class$p.value[6], model_growth_rate_variant_class$p.stars[6], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 0.5))
pdf(paste0(figures_save_path, "ExtendedFigure1G.pdf"), width = 10, height = 7)
ext_fig_tp53
dev.off()
```

# Extended Figure - Lollipop Plots for TP53, PPM1D, CHEK2
...
pdf(paste0(figures_save_path, "ExtendedFigure1A.pdf"), width = 13, height = 8)
ext_fig_tp53_lollipop
dev.off()
...
ext_fig_ppm1d_lollipop <- g1_tp %>%
    ...
    my_lollipop(dat.genetics.unique=., 
                ...
                cutoff.hotspot = 20,
                colors.mutation_type=colors.mutation_type)
pdf(paste0(figures_save_path, "ExtendedFigure1B.pdf"), width = 13, height = 8)
ext_fig_ppm1d_lollipop
dev.off()
...
ext_fig_chek2_lollipop <- g1_tp %>%
    ...
    my_lollipop(dat.genetics.unique=., 
                ...
                cutoff.hotspot = 20,
                colors.mutation_type=colors.mutation_type)
pdf(paste0(figures_save_path, "ExtendedFigure1C.pdf"), width = 13, height = 8)
ext_fig_chek2_lollipop
dev.off()
```
...
pdf(paste0(figures_save_path, "ExtendedFigure1D.pdf"), width = 10, height = 7)
ext_fig_tp53_bubble
dev.off()
...
pdf(paste0(figures_save_path, "ExtendedFigure1E.pdf"), width = 10, height = 7)
ext_fig_ppm1d_bubble
dev.off()
...
ext_fig_chek2_bubble <- df_to_plot_ddr %>%
    ...
    theme(
        legend.position = c(0.60, 0.95),
        plot.margin = margin(0, 0, 0, 01, "cm"),
    ) +
    guides(
        color = guide_legend(byrow = TRUE, order = 1, override.aes = list(size = 5)),
        shape = guide_legend(byrow = TRUE, order = 2, override.aes = list(size = 5)),
...
pdf(paste0(figures_save_path, "ExtendedFigure1F.pdf"), width = 10, height = 7)
ext_fig_chek2_bubble
dev.off()
```
...
g1_tp_tmp <- g1_tp_tmp %>%
    mutate(Trilaciclib = factor(Trilaciclib, levels = c("Trilaciclib", "Placebo")))
# Center
g1_tp_tmp$preVAF_c <- scale(g1_tp_tmp$preVAF, center = TRUE, scale = FALSE)
g1_tp_tmp$Age_c <- scale(g1_tp_tmp$Age, center = TRUE, scale = FALSE)

g1_tp_tmp_ppm1d <- g1_tp_tmp %>% filter(Gene == "PPM1D")
g1_tp_tmp_ppm1d$preVAF_c <- scale(g1_tp_tmp_ppm1d$preVAF, center = TRUE, scale = FALSE)
g1_tp_tmp_ppm1d$Age_c <- scale(g1_tp_tmp_ppm1d$Age, center = TRUE, scale = FALSE)

g1_tp_tmp_tp53 <- g1_tp_tmp %>% filter(Gene == "TP53")
g1_tp_tmp_tp53$preVAF_c <- scale(g1_tp_tmp_tp53$preVAF, center = TRUE, scale = FALSE)
g1_tp_tmp_tp53$Age_c <- scale(g1_tp_tmp_tp53$Age, center = TRUE, scale = FALSE)

g1_tp_tmp_chek2 <- g1_tp_tmp %>% filter(Gene == "CHEK2")
g1_tp_tmp_chek2$preVAF_c <- scale(g1_tp_tmp_chek2$preVAF, center = TRUE, scale = FALSE)
g1_tp_tmp_chek2$Age_c <- scale(g1_tp_tmp_chek2$Age, center = TRUE, scale = FALSE)

g1_tp_tmp_ddr <- g1_tp_tmp %>% filter(Gene_Class == "DDR")
g1_tp_tmp_ddr$preVAF_c <- scale(g1_tp_tmp_ddr$preVAF, center = TRUE, scale = FALSE)
g1_tp_tmp_ddr$Age_c <- scale(g1_tp_tmp_ddr$Age, center = TRUE, scale = FALSE)

glm(
    formula = growth_rate ~ Trilaciclib + preVAF_c + Age_c + Trilaciclib*preVAF_c + Trilaciclib*Age_c,
    data = g1_tp_tmp_ddr
) %>%
...
data_to_plot %>%
    ...
    group_by(Trilaciclib) %>%
    summarise(
        median_vaf_change = median(change_in_VAF),
        average_vaf_change = mean(change_in_VAF),
    ) %>%
    pivot_wider(names_from = Trilaciclib, values_from = c(median_vaf_change, average_vaf_change)) %>%
    mutate(perc_reduction = (1 - median_vaf_change_Trilaciclib / median_vaf_change_Placebo) * 100) %>%
    select(perc_reduction)

# We want to see the effect on the largest clone per gene per person
...
pdf(paste0(figures_save_path, "SupplementalFigure1.svg"), width = 10, height = 8)
supp_fig1
dev.off()
...
supp_fig2a <- df_to_plot %>%
    ...
    scale_y_continuous(trans = scales::pseudo_log_trans(0.001), breaks = c(-0.05, -0.01, -0.005, 0, 0.005, 0.01, 0.05), limit = c(NA, 1.5)) +
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot, 5, 1.75, 2, model_growth_rate_class$p.value[3], model_growth_rate_class$p.stars[3], show_values = TRUE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot, 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 2))
pdf(paste0(figures_save_path, "SupplementalFigure2A.pdf"), width = 10, height = 8)
supp_fig2a
dev.off()
...
supp_fig2b <- df_to_plot %>%
    ...
    scale_y_continuous(trans = scales::pseudo_log_trans(0.001), breaks = c(-0.05, -0.01, -0.005, 0, 0.005, 0.01, 0.05), limit = c(NA, 1.5)) +
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene_Class == "DDR"), 5, 1.75, 2, model_growth_rate_class$p.value[3], model_growth_rate_class$p.stars[3], show_values = TRUE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene_Class == "DDR"), 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene_Class == "DDR"), 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 2))
pdf(paste0(figures_save_path, "SupplementalFigure2B.pdf"), width = 10, height = 8)
supp_fig2b
dev.off()
...
supp_fig2c <- df_to_plot %>%
    ...
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene_Class == "DDR"), 6, 1.75, 2.25, model_growth_rate_class$p.value[4], model_growth_rate_class$p.stars[4], show_values = TRUE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene_Class == "DDR"), 7, 2, 2.25, model_growth_rate_class$p.value[6], model_growth_rate_class$p.stars[6], show_values = TRUE, textsize = 8, size = 2))
pdf(paste0(figures_save_path, "SupplementalFigure2C.pdf"), width = 10, height = 8)
supp_fig2c
dev.off()
```
...
supp_fig2d <- df_to_plot %>%
    ...
    panel_theme_basic +
    theme(
        axis.text.x = element_markdown(size = 28),
        axis.text.y = element_text(size = 28),
        axis.title.x = element_text(size = 32),
        axis.title.y = element_text(size = 32),
        legend.text = element_text(size = 30),
    ...
    scale_y_continuous(trans = scales::pseudo_log_trans(0.001), breaks = c(-0.05, -0.01, -0.005, 0, 0.005, 0.01, 0.05), limit = c(NA, 1.5)) +
    scale_fill_manual(values = treatment_colors) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 5, 0.75, 1, model_growth_rate_gene$p.value[1], model_growth_rate_gene$p.stars[1], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 6, 0.75, 1.25, model_growth_rate_gene$p.value[2], model_growth_rate_gene$p.stars[2], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*TP53*"), 7, 1, 1.25, model_growth_rate_gene$p.value[7], model_growth_rate_gene$p.stars[7], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 5, 1.75, 2, model_growth_rate_gene$p.value[3], model_growth_rate_gene$p.stars[3], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) + 
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 6, 1.75, 2.25, model_growth_rate_gene$p.value[4], model_growth_rate_gene$p.stars[4], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*PPM1D*"), 7.1, 2, 2.25, model_growth_rate_gene$p.value[8], model_growth_rate_gene$p.stars[8], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 5, 2.75, 3, model_growth_rate_gene$p.value[5], model_growth_rate_gene$p.stars[5], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 6, 2.75, 3.25, model_growth_rate_gene$p.value[6], model_growth_rate_gene$p.stars[6], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) +
    do.call(geom_signif, default_sig_data(df_to_plot %>% filter(Gene == "*CHEK2*"), 7.1, 3, 3.25, model_growth_rate_gene$p.value[9], model_growth_rate_gene$p.stars[9], show_values = TRUE, hide_ns = FALSE, textsize = 8, size = 2)) 
pdf(paste0(figures_save_path, "SupplementalFigure2D.pdf"), width = 10, height = 8)
supp_fig2d
dev.off()
```
...
supp_fig3 <- bio_fig4_data %>%
    mutate(
        lower_depth = pmin(average_depth_1, average_depth_2),
        depth_bin = case_when(
            lower_depth < 12000 ~ "<12k",
            lower_depth >= 12000 & lower_depth < 13000 ~ "12k-13k",
            lower_depth >= 13000 & lower_depth < 14000 ~ "13k-14k",
            lower_depth >= 14000 & lower_depth < 15000 ~ "14k-15k",
            lower_depth >= 15000 & lower_depth < 16000 ~ "15k-16k",
            lower_depth >= 16000 & lower_depth < 17000 ~ "16k-17k",
            lower_depth >= 17000 & lower_depth < 18000 ~ "17k-18k",
            lower_depth >= 18000 ~ ">=18k"
        ),
        depth_legend_val = case_when(
            depth_bin == "<12k" ~ 11000,
            depth_bin == "12k-13k" ~ 12000,
            depth_bin == "13k-14k" ~ 13000,
            depth_bin == "14k-15k" ~ 14000,
            depth_bin == "15k-16k" ~ 15000,
            depth_bin == "16k-17k" ~ 16000,
            depth_bin == "17k-18k" ~ 17000,
            depth_bin == ">=18k" ~ 18000
        )
    ) %>%
    ggplot(
        aes(
            x = rep1,
            y = rep2,
            color = depth_legend_val
        )
    ) +
    geom_point(size = 4) +
    scale_color_gradient(
        low = "red", high = "green", name = "Sequencing Depth",
        breaks = c(11000, 12000, 13000, 14000, 15000, 16000, 17000, 18000),
        labels = c("<12k", "12k", "13k", "14k", "15k", "16k", "17k", ">=18k"),
    ) +
    ...
    annotate("text", x = 0.0001, y = 0.055, label = paste0("Pearson r = ", round(pearson_corr, 2)), size = 6, hjust = 0)
pdf(paste0(figures_save_path, "SupplementalFigure3.pdf"), width = 10, height = 8)
supp_fig3
dev.off()
```

# Supplemental Figure 4 - Bioinformatics Figure 4 Coefficient of Variation
```{r supp_fig_4, fig.width=10, fig.height=7}
coefficient_of_variation_no_filter <- bio_fig4_unfiltered_data %>%
    mutate(
        rep2 = ifelse(which_rep == "rep2", ifelse(called_vaf == 0, pileup_vaf, called_vaf), rep2),
        rep1 = ifelse(which_rep == "rep1", ifelse(called_vaf == 0, pileup_vaf, called_vaf), rep1),
        rep1 = as.numeric(rep1),
        rep2 = as.numeric(rep2),
    ) %>%
    filter(rep1 < 0.002 | rep2 < 0.002) %>%
    select(Subject, key, rep1, rep2, average_depth_1, average_depth_2) %>%
    rowwise() %>%
    mutate(
        lower_depth = pmin(average_depth_1, average_depth_2),
        cv = sd(c(rep1, rep2)) / mean(c(rep1, rep2))
    ) %>%
    ungroup() %>%
    select(Subject, key, rep1, rep2, cv, lower_depth)
coefficient_of_variation <- bio_fig4_data %>%
    filter(rep1 < 0.002 | rep2 < 0.002) %>%
    select(Subject, key, rep1, rep2, average_depth_1, average_depth_2) %>%
    rowwise() %>%
    mutate(
...
cv_plot <- coefficient_of_variation_no_filter %>%
  ggplot(
    aes(
      x = lower_depth,
      y = cv
  ...
  geom_vline(xintercept = 17000, linetype = "dashed", color = "red") +
  theme_bw() +
  labs(
    #title = "Coefficient of Variation of VAF Measurements < 0.2%",
    x = "Sequencing Depth",
...
coefficient_of_variation_combined <- rbind(
    g1_tp %>%
        filter(Gene_Class == "DDR") %>%
        filter(
            case_when(
                preVAF < 0.002 | postVAF < 0.002 ~ TRUE,
                Cohort == "TNBC" & (preVAF < 0.002 | duringVAF < 0.002) ~ TRUE,
                TRUE ~ FALSE
            ) 
        )%>%
        select(DeID, Cohort, Trilaciclib, key, Gene, preVAF, duringVAF, postVAF, change_in_days) %>%
        ...
        rowwise() %>%
        mutate(cv = sd(c(Timepoint_1_VAF, Timepoint_2_VAF)) / mean(c(Timepoint_1_VAF, Timepoint_2_VAF))) %>%
        ungroup() %>%
        select(DeID, key, Timepoint_1_VAF, Timepoint_2_VAF, Dataset, cv),
    coefficient_of_variation %>%
        mutate(Dataset = "Chan et al. 2024") %>%
        select(DeID = Subject, key = key, Timepoint_1_VAF = rep1, Timepoint_2_VAF = rep2, Dataset, cv)
)
coefficient_of_variation_combined$Dataset <- factor(coefficient_of_variation_combined$Dataset, levels = c("DDR CH", "Chan et al. 2024"))

cv_stats <- coefficient_of_variation_combined %>%
    glm(
        cv ~ Dataset,
        data = .
    ) %>%
...
cv_combined_plot <- coefficient_of_variation_combined %>%
    ggplot(
        aes(
            x = Dataset,
            y = cv
    ...
    geom_signif(y_position = 1.5, xmin = 1, xmax = 2, annotation = paste0("P=", format(cv_stats$p.value[1], digits = 2)), tip_length = 0.01, textsize = 5)
cv_combined_plot

supp_fig4 <- cowplot::plot_grid(
    cv_plot, cv_combined_plot,
    ncol = 2,
    labels = c("a", "b"),
    label_size = 18
)
pdf(paste0(figures_save_path, "SupplementalFigure4.pdf"), width = 12, height = 8)
supp_fig4
dev.off()
```